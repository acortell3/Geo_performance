...
#######################        THE MODEL IS      ###############################
# Resp ~ Inclination + Distance + Morph1 + Morph2 + Max width + Shaft + Random #
...
## Change response
data$y <- Data$Max_force[-9] ## Max force without outlier
...
## Plot for mean
plot(d_area_mean, main = "Energy", xlab = "mean", 
     ylab = "", col = "white")
...
boxplot(Values ~ Variables, data = pars_area, main = "Energy")
grid()
...
## Compute means and CIs of the parameters' posterior distributions for reporting

## Area
mean(output_lm_area_sh_model$chain1[,2]) # Inc
mean(output_lm_area_sh_model$chain1[,3]) # Dist
mean(output_lm_area_sh_model$chain1[,4]) # M1
mean(output_lm_area_sh_model$chain1[,5]) # M2
mean(output_lm_area_sh_model$chain1[,6]) # W
mean(output_lm_area_sh_model$chain1[,7]) # S
